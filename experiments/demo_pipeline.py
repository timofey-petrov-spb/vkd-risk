...
ДЕМОНСТРАЦИЯ СВЯЗНОСТИ ЗВЕНЬЕВ. НЕ ЭТАЛОН. Разбор Codex 19.09 нашёл ошибки,
которые здесь НЕ исправлены и должны быть исправлены в vkd/:
  * belt_flux складывает дифференциальные потоки в узлах без интегрирования
    по энергии — величина и размерность неверны; нужно интегрирование
    трапециями с явным обращением с хвостом спектра;
  * flux_int: суммирование по времени не даёт заявленных част./см²;
  * вне сетки таблицы возвращается 0 — это «нет модели», а не нулевой поток;
  * B_over_B0 ограничивается снизу единицей — молчаливый кламп, нарушение
    правила 1 договора; несовместимость диполя и IGRF надо показывать;
  * последнее значение GOES применяется к будущим окнам без проверки давности;
  * начало расчёта берётся из текущего времени — для воспроизводимости
    задавать явно;
  * совпадение двух реализаций одной цепочки не доказывает физику: нужны
    независимые эталоны (опубликованные тест-кейсы SGP4, аналитический спектр).
Скрипт оставлен как иллюстрация, что все звенья соединяются. Показывает:
