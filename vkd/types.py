...
ВЕРСИЯ 2.3 (19.09, круг 11, область Б, обратно совместима): поле
MechanismAssessment.declared_common_ru с умолчанием () — общие каналы механизма (R14).
Типы, которые производит и читает Codex (EnvironmentSample, EventInterval, Request,
Manifest), не меняются ни одним полем; правка целиком внутри области Б.
...
class MechanismAssessment:
    ...
    # ОБЩИЕ КАНАЛЫ механизма (R14, решение владельца 19.09): каналы, у которых прогнозного
    # продукта с разрешением по сравниваемому окну не существует в природе, а последнее
    # наблюдение получено и не устарело. Такой канал одинаков для всех сравниваемых окон по
    # построению: он их не различает и обязательную линию не обнуляет. Каждая строка — готовая
    # фраза объявленной области вывода (что именно не сравнивается, каков уровень последнего
    # наблюдения и его время, чего мы не знаем).
    # Поле отдельное, а не подмножество `coverage_notes`, потому что это НЕ «чего не хватает»:
    # заметки о покрытии уходят в причины вердикта, а эта фраза — в объявленную область, и
    # смешивать их нельзя. Отдельное поле ещё и переживает `dataclasses.replace(coverage_notes=())`
    # в проверках, то есть объявленная область не зависит от того, как объявлено покрытие.
    declared_common_ru: tuple[str, ...] = ()
