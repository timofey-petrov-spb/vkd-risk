...
cutoff_GV в точке считается модулем орбиты (A3) и здесь НЕ пересчитывается: методы
разных величин подписаны раздельно в сводке происхождения. С круга 11 её даёт таблица
Ж.1 ОСТ 134-1044-2007, а центральный наклонённый диполь остался запасным путём вне
широт таблицы; чем посчитана точка, записано в ней самой (cutoff_kind).
"""
...
def belt_coordinates(points: list[TrajectoryPoint], coeff_path: str) -> tuple[list[TrajectoryPoint], dict]:
    ...
    return out, {'method': 'eccentric_dipole', 'coefficients': str(coeff_path), 'epoch_utc': when.isoformat(), 'position_conversion': 'WGS84 geodetic to ECEF',
                 ...
                 # Круг 11: обрезание считает модуль орбиты (A3), но уже не диполем — таблицей Ж.1 ОСТ,
                 # с центральным диполем как запасным путём. Прежняя подпись называла его дипольным и
                 # после слияния противоречила бы ограничениям модуля орбиты, стоящим на том же экране.
                 'cutoff_GV_method': 'вертикальная жёсткость обрезания приходит из модуля орбиты (A3) и НЕ пересчитана '
                                     'эксцентричным диполем: это отдельная модель — таблица Ж.1 ОСТ 134-1044-2007, '
                                     'запасной путь центральный наклонённый диполь; подписана раздельно (R11), '
                                     'чем посчитана точка — в ней самой (cutoff_kind)',
