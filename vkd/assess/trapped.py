...
построчно (data/ost1044_belts/README.md).

Метод:
  * по L — линейная интерполяция между соседними табличными оболочками;
    вне диапазона таблицы — статус «нет модели», не ноль;
  * по B/B0 — линейная интерполяция внутри оболочки; выше максимального
    табличного B/B0 — физический ноль (точка отражения выше), статус
    «за зеркальной точкой»; B/B0 < 1 (несовместимость дипольной L и поля
    IGRF) не клампится: берётся экваториальное значение и ставится статус;
...
class BeltTable:
    ...
    def _spectrum_at(self, L_row: float, B_over_B0: float) -> Optional[np.ndarray]:
        ...
        return pts[j - 1][1] * (1 - w) + pts[j][1] * w
    ...
    def integral_flux(self, L: Optional[float], B_over_B0: Optional[float], e_min_MeV: float) -> FluxResult:
        ...
        status = 'ok'
        if B_over_B0 < 1.0:
            status, B_over_B0 = 'inconsistent_BB0', 1.0
        ...
        if j == 0 or self.Ls[j - 1] == L:
            ...
            w = (L - L0) / (L1 - L0)
            rows = [(L0, 1 - w), (L1, w)]
        total, any_model = 0.0, False
        for L_row, wt in rows:
            spec = self._spectrum_at(float(L_row), B_over_B0)
            if spec is None:
                continue                      # за зеркальной точкой на этой оболочке: вклад 0
            any_model = True
            total += wt * integrate_power_law(self.energies_MeV, spec, e_min_MeV)
        if not any_model:
