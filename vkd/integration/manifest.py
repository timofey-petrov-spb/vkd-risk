...
def collect_records(raw_records, history_versions, orbit_provenance, belts, root):
    ...
    # Техногенный мусор: фактор называет модель, значит таблицы стандарта обязаны лежать
    # в архиве — иначе повтор без сети не воспроизводит число попаданий.
    from vkd.assess.debris import record_files as debris_record_files
    for rid, path, meta in debris_record_files():
        add_file(rid, dict(meta, raw_path=path), root/path)
