...
def recommend(assessments: Sequence[WindowAssessment], th: Thresholds) -> Recommendation:
    ...
    if len(candidates) == 1:
        ...
        return Recommendation(preferred=c.window, verdict='preferred', rule_applied=rule,
                              per_mechanism_comparison=per_one,
                              reasons=tuple(per_one.values()) + cond_reasons + note_partial, tolerance_basis=tol)
    if equiv_set:
