...
from vkd.assess.seasonal import seasonal_hits_track, prepare_seasonal_track, MODEL_ID, CATALOGUE_ID, METHOD_ID
...
def run(mode: str, t0: datetime, duration_min: int, search_min: int, window_offsets_min: list[int],
    ...
    if disabled.get('goes') == 'off':
        # Ряд наблюдений бывает только там, где его читают из архива (разбор). В текущем режиме
        # его нет вовсе — None, — и превращать None в пустой список нельзя: слой сравнения по
        # пустому СПИСКУ уходит в архивную ветку и печатает «архивные наблюдения покрывают 0 %
        # окна» там, где никакого архива не спрашивали. Причина отказа обязана называть то, что
        # случилось на самом деле: пользователь исключил источник (круг 11, сохранённый пример
        # с отказом — единственное место, где эта строка попадает людям на глаза).
        if goes_observations is not None:
            goes_observations = []
        goes_absent_ru = goes_absent_ru or 'источник исключён пользователем, он не опрашивался'
    ...
    if traj:
        ...
        orb.provenance.setdefault('limitations', []).append(
            ...
            'Вертикальная жёсткость обрезания cutoff_GV — ТРЕТЬЯ модель, не та, что L и B/B0: с круга 11 её '
            'даёт таблица Ж.1 ОСТ 134-1044-2007, а центральный наклонённый диполь остался запасным путём; '
            'чем посчитана каждая точка и какова область применимости — отдельными строками ограничений '
            'модуля орбиты ниже. За одну систему координат эти модели не выдаются.'
    ...
    seasonal_results = {}
    prepared_meteors = None
    try:
        prepared_meteors = prepare_seasonal_track(
            [p.t_utc for p in traj], [p.alt_km for p in traj],
            (orb.provenance or {}).get('inertial_states') or {})
    except ValueError:
        pass  # Assess each contiguous window separately; no blanket zero or fallback mean.
    def _mmod(w: Window):
        ...
        try:
            ...
            def direct_result():
                return seasonal_hits_track(
                    [p.t_utc for p in pts], [p.alt_km for p in pts],
                    (orb.provenance or {}).get('inertial_states') or {}, 1.0, 1e-3)
            if prepared_meteors:
                try:
                    result = prepared_meteors(w.start_utc, end)
                except ValueError:
                    result = direct_result()  # A bad segment elsewhere must not erase this window.
            else:
    ...
    def _seasonal_of(w: Window):
        _mmod_of(w)
        return seasonal_results[w.start_utc.isoformat()]
