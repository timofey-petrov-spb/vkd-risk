...
цвет означает происхождение величины, а не «хорошо/плохо». Четыре тона и один служебный:
  стальной  --calc — наш расчёт;
  хвойный   --obs — наблюдение (в том числе взятое из кеша: давность объявляется подписью, а не цветом);
  латунный  --fc — внешний прогноз, и только он;
  кирпичный --cond — условие проверки, аномалия, отказ источника;
  серый     --none — данных нет.

Почему набор именно такой
-------------------------
Прежние четыре тона (#7ab8f5, #5ed39a, #e7b45c, #f58b7f) владелец опознал с одного взгляда как
машинные, и он прав: это ровно тот набор, который выдаёт любой генератор тёмной темы. У него
две приметы, и обе измеримы.

Первая — насыщенность высокая и почти общая: 0,86 / 0,57 / 0,74 / 0,86 по HSL. Вторая, и
главная, — тона различались почти ТОЛЬКО тоном. Переведённые в серый полутон (серый той же
относительной яркости), они давали #b3b3b3, #bebebe, #bdbdbd и #a9a9a9: наблюдение и внешний
прогноз расходились на 1,8 % относительной яркости, то есть в серой печати, на плохом
проекторе и для дальтоника сливались в один. Цвет нёс всю нагрузку один.

Новый набор разведён ПО СВЕТЛОТЕ, а тон оставлен приметой второго порядка:

    назначение       тон         цвет     серый    яркость Y  насыщенность  контраст к фону
    условие/отказ    кирпичный   #c17d68  #8e8e8e  0,2700     0,42          6,07
    наблюдение       хвойный     #76a78e  #9d9d9d  0,3344     0,22          7,29
    внешний прогноз  латунный    #bba889  #ababab  0,4038     0,27          8,61
    наш расчёт       стальной    #a1bed5  #bbbbbb  0,4921     0,38          10,28

Шаги по относительной яркости — +23,8 %, +20,7 %, +21,9 %, размах между крайними +82,2 %.
Четыре полутона серого читаются как четыре разных уровня, и это проверяется не на глаз:
серый считается по той же формуле WCAG, что и контраст (app/backdrop.relative_luminance).

Насыщенность не только НИЗКАЯ (была 0,57…0,86, стала 0,22…0,42), но и РАЗНАЯ, и порядок у неё
осмысленный: громче всех условие проверки — его пропускать нельзя; следом наш собственный
расчёт; тише — чужой прогноз; тише всех наблюдение, потому что наблюдений на экране больше
всего, и ровный зелёный шум как раз и читается как машинная работа.

Фон опущен с #0e1117 до #08090b: глубже и почти нейтральный (прежний заметно синил — 14/17/23
по составляющим против нынешних 8/9/11). Девять десятых экрана — серое; цвет остаётся только
на самих величинах. Единственное крупное цветное пятно на экране — полоса решения, и это
сделано формой, а не тоном: см. раздел о двух языках цвета ниже по файлу.
...
/* ТЁМНАЯ тема. Палитра означает то же, что и прежде, — ПРОИСХОЖДЕНИЕ величины, — но собрана
   заново; почему именно так и какими числами она разведена по светлоте и насыщенности,
   написано в описании модуля, а не здесь: комментарий CSS уходит в браузер при КАЖДОЙ
   отрисовке, а читают его разработчики. Здесь остаются только числа, которые обязаны стоять
   рядом с самими значениями, — контраст. Он проверен расчётом по формуле относительной
   яркости, а не на глаз (фон страницы #08090b, фон панели #101217; порог — 4,5:1):
...
METHOD_BLOCKS = [
    ...
    {'no': 11, 'group': 'Сезонные потоки и мусор',
     'title': 'Сезонный вклад метеорных потоков',
     'latex': r'N = N_{sp} + \sum_i A \int_{t_0}^{t_1} F_i(t)\,dt',
     'symbols': 'N_sp — спорадический фон Grün ПОСЛЕ вычитания среднегодового вклада потоков, чтобы не '
                'считать их дважды; F_i — поток i-го потока на дату: профиль активности по солнечной долготе, '
                'положение и дрейф радианта, относительная скорость встречи со станцией, затенение Землёй.',
     'source': 'ECSS-E-ST-10-04C Rev.1, таблица C-2 — 49 потоков; реализация ecss-seasonal-engineering-v1.',
     'limits': 'Всплески конкретного года модель не предсказывает: это климатология. Нормировка каталога и '
               'эпоха радиантов проверены как альтернативные гипотезы; при изменении сравнительного вывода '
               'однозначный выбор не выдаётся. Абсолютная калибровка потока по орбитальным измерениям не сделана.'},
    {'no': 12, 'group': 'Сезонные потоки и мусор',
     'title': 'Техногенное вещество',
     'latex': r'N_j = C_N\,S\,Q_j(h,i)\,[F(t_1) - F(t_0)]',
     'symbols': 'Q_j — плотность потока относительно аппарата для диапазона размеров j, м⁻²·год⁻¹; S — площадь '
                'сечения, для односторонней случайно ориентированной пластины 1 м² это 0,25 м²; C_N = 1 для '
                'изотропного тела; F(t) — функция прогноза засорённости по годам.',
     'source': 'ГОСТ Р 25645.167-2005: таблица 7.2, эпоха 2000 г.; формулы 2 и 6; таблицы 8.1—8.16.',
     'limits': 'Поток усреднён по витку и от направления не зависит, поэтому при равной высоте и длительности '
               'он ОДИНАКОВ у всех окон и окно им не выбирается. Наклонение станции 51,6° лежит НИЖЕ сетки '
               'таблицы, которая начинается с 55°, — берётся край сетки. Модель построена до разрушения '
               'Фэнъюнь-1С 2007 г. и столкновения Иридиум—Космос 2009 г., прогноз доведён до 2025 г.'},
    {'no': 13, 'group': 'Доза за защитой',
     'title': 'Поглощённая доза за защитой скафандра',
     'latex': r'D = 10\,D_K(d,h,i)\,\Phi\,/\,(\langle\varphi\rangle\,T_{10})',
     'symbols': 'D_K — доза за 10 лет из таблиц приложения К, рад(Si), за защитой толщиной d в форме '
                'полусферы; Φ — флюенс окна из формулы (1); ⟨φ⟩ — средний поток на той же орбите, '
                'посчитанный тем же кодом; T_10 — десять лет в секундах; множитель 10 переводит рад в мГр.',
     'source': 'ОСТ 134-1044-2007, приложение К, таблицы К.2.5 и К.2.6; узлы толщины 0,5 / 1,0 / 1,5 г/см² '
               'берутся без интерполяции, по высоте логарифмическая, по наклонению линейная между 30° и 60°.',
     'limits': 'Доза ТОЛЬКО от захваченных протонов пояса: солнечные и галактические частицы в неё не входят, '
               'и в протонное событие настоящая доза выше. Это поглощённая доза в кремнии за объявленной '
               'толщиной, а не эквивалентная доза органа и не предел по нормам; переход к ткани требует '
               'отношения тормозных способностей, которого в первоисточниках нет. Защита принята однородной '
               'по телу, реальная зависит от направления и части тела.'},
]
