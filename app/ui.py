...
METHOD_BLOCKS = [
    {'no': 1, 'group': 'Захваченные протоны',
     ...
     'limits': 'Хвост выше E_max = 300 МэВ отброшен и объявлен. Вне сетки L = 1,14…9 модели нет — это «нет модели», '
               'а не нуль. Вне B/B0 любой нужной строки таблицы поток также неизвестен; конец таблицы не доказывает точку отражения. '
               'Интерполируется дифференциальный спектр, затем интегрируется энергия. Двухточечная схема — приближение к прил. Е. '
               'Разрывы свыше 60 с не заполняются, за край траектории расчёт не продолжается. При неполном охвате показан только известный вклад, не полный флюенс.'},
