...
def verdict_panel(rec, S: dict, windows_ru: dict, assessments=None, pro: bool = False,
    ...
    # Двенадцатый круг, пункт 2: на профессиональном уровне подробная форма тоже уезжает в свёртку.
    # Она повторяет короткую, только длиннее: на снимке 20 это 470 знаков сразу под 140-знаковой
    # строкой о том же самом — два сообщения об одном (бриф §9.7). Ничего не потеряно: свёртка
    # стоит в этом же блоке и на этом же уровне.
    ...
    if pro:
        ...
        _ORDER = {'cond': 0, 'cmp': 1, 'need': 2, 'other': 3}
...
def ribbon_caption_ru(scan: dict, e_min_MeV=None, duration_min=None) -> str:
    ...
    означает точка. Ни единицы, ни порядка величин здесь нет — они стоят на самих осях.
    """
    ...
    return ('Чем ниже кривая, тем меньше набранный поток частиц %s; синяя полоса — рекомендованный '
            'промежуток, точка — худшее время на сроке.' % dur_ru)
