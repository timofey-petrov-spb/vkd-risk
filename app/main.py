...
with st.sidebar:
    ...
    with st.expander('Вид', expanded=False, icon=':material/visibility:'):
        ...
        # Строки «Тема тёмная, светлой нет.» здесь нет и не будет: владелец показал на неё
        # пальцем. Переключателя тем нет, и молчание об этом — не пробел: человек видит один
        # экран и не ищет второй. Почему тема тёмная, записано в .streamlit/config.toml, где
        # она и задана, — то есть там, где это читает разработчик, а не зритель.
