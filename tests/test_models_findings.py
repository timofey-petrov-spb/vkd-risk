...
def test_review_window_beyond_archive_end_names_the_reason():
    ...
    # Причина отказа — именно конец архива уведомлений, а не частичная линия метеороидов:
    # частичное покрытие с 19.09 отказа не даёт, оно объявляется областью вывода (Codex п. 1).
    assert any('архива' in m or 'DONKI' in m for m in r.rec.missing), r.rec.missing
    assert not any('сезонный вклад' in m for m in r.rec.missing), r.rec.missing
...
def test_sep_level_from_donki_body_and_storm_signals_grouped():
    ...
    # 19.09: частичное покрытие обязательной линии больше не даёт отказа (решение владельца по
    # разбору Codex п. 1), поэтому вердикт здесь определяют УСЛОВИЯ: оба окна под условием.
    assert r.rec.verdict == 'all_need_check' and storm and sep
    assert r.rec.preferred is None
