...
Что проверяется:
  ...
  * в разметке нет знаков вне печатного диапазона (ловушка восьмеричных escape), нет градиентов,
    нет эмодзи;
  * вес разметки меньше 40 КБ и назван числом, а пояснения не переезжают из исходника в CSS;
  * небо задаёт .stApp ТОЛЬКО фон: ни position, ни z-index, ни псевдоэлемента (первая редакция
    так обнулила высоту .stApp, и overflow срезал всё содержимое страницы);
  * подключение фона не двигает страницу: лишний отступ от второго блока стилей снимается,
    первый остаётся на месте;
  * подсветка изменившейся ячейки помечает ровно изменившиеся ячейки, работает на НАСТОЯЩЕЙ
    разметке ui.panel() и падает, если структура ячейки в app/ui.py изменилась;
  * при слишком малом поле, где яркому классу достаётся ноль звёзд, модуль падает, а не отдаёт
    небо без ярких звёзд — на их наличии держится расчёт наихудшего контраста.
"""
