...
Что проверяется:
  ...
  * сетка |B| глобуса и точки плоской карты (app.viz.saa_grid) — одно и то же поле и один порог;
...
def test_saa_field_is_the_same_field_as_flat_map(field):
    """Сетка |B| глобуса и точки плоской карты — одно поле, один шаг, один порог:
    область на сфере и область на карте не могут разойтись."""
    ...
    assert ours == flat, 'порог |B| по IGRF даёт на глобусе те же узлы, что и на плоской карте'
