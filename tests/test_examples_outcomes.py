...
def test_primery_pokazyvayut_ne_tolko_otkaz():
    ...
    Отказ при этом никуда не делся и проверяется там, где он настоящий: `tests/test_common_channel.py`
    держит все три случая полного отказа (наблюдения нет вовсе, наблюдение устарело сверх предела,
    пуста другая обязательная линия), `tests/test_compare.py` — исключённый пользователем источник.
    Чего не хватает: сохранённого ПРИМЕРА с исключённым источником, чтобы отказ был виден и на
    сдаче, а не только в проверках. Он требует правки `scripts/make_examples.py` (добавить случай
    с `disabled={'goes': 'off'}`) — это вне области круга, запись оставлена в `journal/tim.md`.
    """
    v = verdicts()
    assert v, 'примеров нет — сначала python scripts/make_examples.py'
    assert set(v.values()) >= {'preferred', 'equivalent', 'all_need_check'}, v
    # исход отказа на примерах не показан — и это записано вслух, а не замолчано пустым множеством
    assert 'insufficient' not in set(v.values()), (
        'отказ снова появился на сохранённом примере — проверьте, не вернулось ли структурное '
        'обнуление обязательной линии пустым каналом: %s' % v)
