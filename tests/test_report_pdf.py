...
def test_pdf_sobiraetsya_na_kazhdom_sohranyonnom_primere(name):
    ...
    # Замер 19.09 по всем одиннадцати сохранённым примерам: от 6 страниц (отказ, 73 начала) до
    # 11 (исторический разбор с условиями). Границы поставлены с запасом в обе стороны: меньше
    # трёх страниц — это не шесть разделов с таблицами, больше двадцати — сорванная вёрстка.
